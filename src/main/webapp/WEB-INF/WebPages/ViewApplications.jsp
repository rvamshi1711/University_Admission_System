                ...
                <div id="demo">
                        <table class="table table-striped table-hover">
					        <thead>
					         ...
					         int i=0;
					         int n=0;
					         int col=Integer.parseInt(session.getAttribute("col").toString());
					        // int k = Integer.parseInt(session.getAttribute("k").toString());
					         %>
					        <c:forEach items="${list}" var="cs1">
							   <%
							   if(i==0)
...
<form method="post" action="coursedd">
<td>
<%
		int j=0;
		
		%>
		<c:forEach items="${em}" var="cs2">	

					<%
					
					if(j==n){
						%>
						<input type="hidden"  name="emailid" value="${cs2 }" >
						<%
						n++;
						break; 
					}
...
<td>
<select class="form-select" name="coursedropdown" >
		
						
		<c:forEach var="v" items="${branch}">
